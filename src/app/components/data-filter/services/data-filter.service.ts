...
import {
  ...
  LocalStorageService, ORGANISATION_UNIT_KEY, PROGRAM_INDICATOR_KEY, PROGRAM_KEY
...
export class DataFilterService {
  ...
  getPrograms(): Observable<any[]> {
    return this.http.get('../../../api/programs.json?paging=false&fields=id,name,programType,programIndicators[id,name')
      .map(res => res.json().programs || [])

  }

  getProgramIndicators(): Observable<any[]> {
    return this.http.get('../../../api/programIndicators.json?paging=false&fields=id,name')
      .map(res => res.json().programIndicators || [])
